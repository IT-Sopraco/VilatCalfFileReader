...
namespace VSM.RUMA.SRV.FILEREADER.SOPRACO
{
    ...
    public class SopracoTXTPlugin : IReaderPlugin
    {
        ...
        public int LeesFile(int pThrId, DBConnectionToken pAgroFactuurToken, int pProgramID, string pAgrobaseUser, string pAgrobasePassword, int FileLogId, string Bestandsnaam)
        {
            ...
            unLogger.WriteInfo($"{nameof(SopracoTXTPlugin)}.{nameof(LeesFile)} '{Bestandsnaam}' begonnen.");
            ...
            {
                unLogger.WriteError($"{nameof(SopracoTXTPlugin)}.{nameof(LeesFile)} Kan bestandsType niet achterhalen voor bestand: '{Bestandsnaam}'.");
                return -2;
            }
            try
            {
                ...
                unLogger.WriteInfo($"{nameof(SopracoTXTPlugin)}.{nameof(LeesFile)} '{Bestandsnaam}' verwerkt?.");
            }
            catch (Exception ex)
            {
                unLogger.WriteError($"{nameof(SopracoTXTPlugin)}.{nameof(LeesFile)} '{Bestandsnaam}' SopracoTXTImport ex: {ex.ToString()}", ex);
                return -2;
            }
